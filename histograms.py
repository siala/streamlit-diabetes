...
from lab_helpers_diabetes import load_data, CLASS_NAMES, COLOURS, FEATURES

df = load_data()["train"]
groups = {name: df[df["class"] == value] for value, name in CLASS_NAMES.items()}

st.write("The use case we consider here is comparing one count between readmitted and other patients.")
